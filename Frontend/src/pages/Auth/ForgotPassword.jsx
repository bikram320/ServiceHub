...
const ForgotPassword = () => {
    ...
    const role = location.state?.role || "user"; // Get role from navigation state, default to "user"

    // Debug logging
    console.log('ForgotPassword component loaded with role:', role);
    ...
    const handleIdentifyUser = async (e) => {
        ...
        try {
            ...
            // Updated URL to match backend endpoint with query parameter and role
            const response = await fetch(`http://localhost:8080/auth/login/${role}/forget-password?email=${encodeURIComponent(formData.email)}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
    ...
    // Step 2: Verify OTP from Modal
    const handleOTPVerify = async (otpCode) => {
        console.log('OTP Verification started with code:', otpCode);
        try {
            // Updated URL to match backend endpoint and request body format with role
            const response = await fetch(`http://localhost:8080/auth/login/${role}/forget-password/verify-otp`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    email: formData.email,
            ...
            setOtpToken(data.resetToken || data.token || 'verified'); // Store reset token from response
            setShowOTPModal(false);
            setStep('reset'); // Move to reset password step
    ...
    const handleResendOTP = async () => {
        try {
            // Updated URL to match backend endpoint with query parameter
            const response = await fetch(`http://localhost:8080/auth/login/user/forget-password?email=${encodeURIComponent(formData.email)}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            // Updated URL to match backend endpoint and request body format with role
            const response = await fetch(`http://localhost:8080/auth/login/${role}/forget-password/reset-password`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    email,
    ...
    const renderIdentifyStep = () => (
        <div className={styles["forgot-password-content"]}>
            ...
            <button
                className={styles["back-link"]}
                onClick={() => navigate("/LoginSignup", { state: { role } })}
            >
                <ArrowLeft size={16} />
                Back to Login
            </button>
            ...
            <div className={styles["success-section"]}>
                ...
                <button
                    ...
                    onClick={() => {
                        ...
                        navigate("/LoginSignup", { state: { role } });
                    }}
                >
                    Back to Login
                </button>
